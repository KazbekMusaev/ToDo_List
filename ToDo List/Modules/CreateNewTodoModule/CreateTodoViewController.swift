...
final class CreateTodoViewController: UIViewController {
    ...
    private lazy var closeAction = UIAction { [weak self] _ in
        ...
        presenter?.createNewTodo(label: labelTextField.text, text: textView.text ?? "")
    }
}
...
extension CreateTodoViewController: CreateTodoViewProtocol {
    func configureScreen(date: Date, label: String, text: String) {
        settupView()
        dateLabel.text = date.getStr()
        labelTextField.placeholder = label
        placeholderForTextView.text = text
    }
}
